<x-layouts.admin title="Ubah Fasilitas | Admin | {{ config('app.name') }}">
    ...
    <form action="{{ route(config('route.admin.facility.edit-action'), ['facility' => $facility]) }}" method="POST"
        ...
        <x-inputs.image label="Foto Fasilitas" name="image" value="/storage/{{ $facility->image }}" required="true" />

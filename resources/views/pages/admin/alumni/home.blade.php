<x-layouts.admin title="Alumni | Admin | {{ config('app.name') }}" x-data="{
    ...
    <div class="flex gap-3">
        ...
        <x-links.add href="{{ route(config('route.admin.alumni.add')) }}" />
    </div>
    ...
    <x-modals.delete
        x-bind:href="selectedID ? `{{ route(config('route.admin.alumni.delete-action'), ['alumni' => '###']) }}`.replace('###',
            selectedID) : ''" />
